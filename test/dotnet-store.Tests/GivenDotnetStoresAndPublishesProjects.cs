...
namespace Microsoft.DotNet.Cli.Publish.Tests
{
    public class GivenDotnetPublishPublisheswithCacheProjects : TestBase
    {
        ...
        public void AppFailsDueToMissingCache()
        {
            var testAppName = "NewtonSoftDependentProject";
            var profileProjectName = "NewtonsoftProfile";
            var runtimeStoreManifestFile = "NewtonsoftFilterProfile.xml";
            ...
            var testProjectDirectory = testInstance.Root.ToString();
            var profileProjectPath = TestAssets.Get(profileProjectName).Root.FullName;
            var profileFilter = Path.Combine(profileProjectPath, runtimeStoreManifestFile);
            ...
            new DotnetCommand()
                .ExecuteWithCapturedOutput(outputDll)
                .Should().Fail()
                .And.HaveStdErrContaining($"Error: assembly specified in the dependencies manifest was not found probably due to missing runtime store associated with {runtimeStoreManifestFile} -- package: 'Newtonsoft.Json',");
        }
